...
def main():
	...
	qp = QueryParser(filename='../text-test/queries-fiqa-full.txt')
	qp.parse()
	...
	for query_id, query in queries:
		...
		cp = CorpusParser(
			filename='../data/{}_corpus_fiqa.txt'.format(query_id))
		cp.parse()
		corpus = cp.get_corpus()
		proc = QueryProcessor([(query_id, query)], corpus)
		results = proc.run()
